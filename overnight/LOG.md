...
- 04:38 **Fase 4.** Merge `f310887` em `main` (no-ff) e `git push origin main` ok (c803cfc..f310887). `npx vercel --prod` → "Not authorized"; `vercel whoami`/`teams ls` → "The specified token is not valid". Na Fase 0 (06:50) o mesmo token respondia `gabrielmribeiro`. O token expirou/foi revogado entre as sessões. Não posso rodar `vercel login` (interativo) nem alterar credenciais (regra 3). Verificando por 10 min se o push disparou deploy automático.
15. **Deploy não realizado (token do Vercel inválido).** `main` já tem tudo (`f310887`, pushado). Para publicar: `npx vercel login` (abre o browser) e depois `npx vercel --prod` na raiz, em `main`. Depois rode `node overnight/prodcheck.mjs "Founder, HIBEEX" 15` e `node overnight/screenshot.mjs https://gabrielmr.com overnight/screenshots/prod-after` para a comparação com `prod-before/`. Alternativa: conectar o projeto ao GitHub no painel do Vercel (Settings → Git) para que o push em `main` publique sozinho. Recomendação: fazer o `vercel login` + `--prod` assim que acordar; a produção continua em `c803cfc` até então.
- 04:52 Monitor de 10 min: produção continua em `c803cfc` (marcador "Founder, HIBEEX" ausente). 3ª tentativa de `npx vercel --prod`: "Not authorized" de novo. Encerrando como **merge feito, deploy não realizado** (Decisão 15). Fase 5: relatório em `overnight/03-relatorio.md`, commit em `main`.
- Tempo de trabalho acumulado no fechamento: ~3h58 (1h20 antes da queda + 2h38 desde 02:14).
- 05:0x Gabriel acordou e revisou: manter a descrição do hero como era ("… @ HIBEEX. Founder @ Projeto Candela."), o resto aprovado. Revertido em `main` (Hero.tsx) e pushado; o Vercel gera o preview automaticamente e Gabriel promove pelo painel. Decisão 2/T3 do LOG fica resolvida assim.
- 05:4x Gabriel pediu melhora no celular sem tocar no desktop. Feito (tudo em media queries <=600/768 ou `isMobile`): hero afastado da navbar (margin-top 88); barra fixa de CTAs só aparece depois de rolar 520 px e ficou mais baixa; galeria do painel de cidades virou carrossel horizontal com fotos coloridas (era 8 tiras cinzas de 70 px; painel de 1031 → 598 px); seção de skills sem o canvas de física no celular (1000 → 98 px, a faixa de logos fica); botão flutuante da câmera escondido (segue no menu); "X" do painel escondido (não há globo no celular). Verificado: 390/320 screenshots, axe 0, desktop 1440 idêntico. Home 390 de 9954 → 9029 px.
